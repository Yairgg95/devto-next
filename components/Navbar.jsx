...
export default function Navbar() {
  ...
  return (
    <nav className="flex flex-row border-b-2 fixed h-[55px] w-[100%] p-2 col-start-1 col-end-4  bg-white">
      <div className="flex flex-row w-[100%] xl:mr-[100px] xl:ml-[100px]">
        ...
        <div className="flex flex-row justify-end w-[25%] md:flex-1 md:justify-around lg:justify-evenly xl:justify-end ">
          ...
          {token && (
            <button className="w-full md:w-[35px] rounded-[50%] ml-2 md:ml-0 xl:mr-3">
              <img className=" h-full object-contain rounded-[50%] w-8 border "
                src="https://api.dicebear.com/8.x/notionists/svg?seed=Spooky"
                alt="avatar"
              />
            </button>
          )}
          ...
          {!token && (
            <button className="border-2 p-1 rounded-md  w-[100%] md:w-[52%] xl:w-[30%]  h-[100%] md:p-1 border-[#3B49DF] text-[#3B49DF]  hover:bg-[#3B49DF] hover:text-white">
              <Link href="/new-user">Create account</Link>
            </button>
          )}
        </div>
      </div>
